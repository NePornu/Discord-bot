...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
    <div>
        <h1 class="title">Přehled Serveru</h1>
        <p style="color: var(--text-secondary);">Statistiky serveru v reálném čase</p>
    </div>
        ...
        <form method="GET" action="/"
            ...
            <div>
                ...
                <select name="role_id"
                    ...
                    <option value="all" {% if selected_role=='all' %}selected{% endif %}>Všechny role</option>
...
<div id="hidden-drawer" class="hidden-drawer"
    ...
    <h3 style="margin-bottom: 15px; color: var(--text-secondary);">Skryté widgety (přetáhněte zpět pro zobrazení)</h3>
...
<div class="card stat-card" data-id="kpi_online">
    <div class="stat-label">
        Online Nyní
        <span class="info-icon dash-tooltip">? <span class="dash-tooltip-text">Počet uživatelů
                online/active.</span></span>
    </div>
    <div class="stat-value" id="stat-online">{{ realtime_active }}</div>
    <div class="stat-diff diff-positive">V reálném čase</div>
</div>
{% elif id == 'kpi_members' %}
<div class="card stat-card" data-id="kpi_members">
    <div class="stat-label">
        Členů celkem
        <span class="info-icon dash-tooltip">? <span class="dash-tooltip-text">Celkový počet uživatelů.</span></span>
    </div>
...
<div class="card stat-card" data-id="kpi_dau">
    <div class="stat-label">
        Průměrné DAU
        <span class="info-icon dash-tooltip">? <span class="dash-tooltip-text">Denně aktivní uživatelé.</span></span>
    </div>
    <div class="stat-value" id="stat-dau">{{ avg_dau }}</div>
    <div class="stat-diff diff-neutral">Denně aktivní</div>
</div>
{% elif id == 'kpi_churn' %}
<div class="card stat-card" data-id="kpi_churn">
    <div class="stat-label">
        Míra odchodů
        <span class="info-icon dash-tooltip">? <span class="dash-tooltip-text">Procento odchodů za období.</span></span>
    </div>
    <div class="stat-value" id="stat-churn">{{ churn_rate }}%</div>
    <div class="stat-diff diff-negative">Měsíční</div>
</div>
{% elif id == 'security_score' %}
<div class="card stat-card" data-id="security_score" id="security-score-card">
    <div class="stat-label">
        🛡️ Skóre bezpečnosti
        <span class="info-icon dash-tooltip">? <span class="dash-tooltip-text">Celkové hodnocení
                zabezpečení.</span></span>
    </div>
    <div style="display: flex; align-items: center; gap: 12px;">
        ...
        <div style="font-size: 12px; color: var(--text-secondary); line-height: 1.4;">
            <div id="security-verdict" style="color: white; font-weight: 600;">Načítání...</div>
    ...
    <div id="security-insights"
        ...
        <div
            ...
            Postřehy</div>
        <div id="insights-list" style="display: flex; flex-direction: column; gap: 8px; color: var(--text-secondary); max-height: 200px; overflow-y: auto; padding-right: 4px;">
        </div>
    </div>
    <!-- Breakdown tooltip -->
    <div id="security-breakdown"
        ...
        <div style="font-weight: 700; margin-bottom: 12px; font-size: 0.95rem; color: white;">Rozklad skóre</div>
...
<div class="card" data-id="chart_growth" style="min-height: 350px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h3 style="margin: 0;">📈 Růst členů</h3>
        <div style="display: flex; gap: 10px;">
            <button class="chart-toggle active" data-chart="growth" data-type="total">Celkem</button>
            <button class="chart-toggle" data-chart="growth" data-type="joins">Přírůstky</button>
        </div>
    </div>
...
<div class="card" data-id="response_stats">
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px;">
        <div style="text-align:center;">
            <div class="stat-label">Průměrná délka</div>
            <div class="stat-value" style="font-size: 1.5rem;">{{ avg_msg_len }}</div>
        </div>
        <div style="text-align:center;">
            <div class="stat-label">Nejaktivnější den</div>
            <div class="stat-value" style="font-size: 1.5rem;">{{ peak_day }}</div>
        </div>
        <div style="text-align:center;">
            <div class="stat-label">Podíl odpovědí</div>
...
<!-- Main Dashboard Grid -->
<div id="dashboard-grid" class="grid" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px;">
...
<script>
    ...
    function toggleEditMode() {
        if (sortableMain) {
            ...
            editBtn.innerHTML = '🖊️ Upravit rozložení';
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        try {
            ...
            if (data.overall_score !== undefined) {
                ...
                if (ratingEl) {
                    ratingEl.textContent = data.rating || 'Neznámý';
                    ratingEl.style.color = data.rating_color;
                }
                if (detailEl) {
                    detailEl.textContent = 'Najetím zobrazíte detaily';
...
with open("/root/discord-bot/web/frontend/templates/index.html", "w") as f:
    f.write(content)
