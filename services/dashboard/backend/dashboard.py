...
import uuid
import asyncio
from services.dashboard.backend.generator_utils import generate_local_scenario, BASE_TEMPLATES
try:
    from shared.python.config.dashboard_secrets import (
...
async def background_scenario_generator():
    await asyncio.sleep(5) # Let the server start fully
    while True:
        try:
            ...
            for template_idx in range(6):
                ...
                if pool_size < 50: # Keep up to 50 pregenerated per template
                    print(f"[AI Generator] Template {template_idx} pool has {pool_size}/50. Pre-generating variant...")
                    scenario = await generate_local_scenario(template_idx)
                    if "error" not in scenario:
                        await r.rpush(pool_key, json.dumps(scenario))
                        print(f"[AI Generator] Successfully pregenerated variant for template {template_idx}: {scenario.get('title')}")
                        generated_any = True
                        break # Only generate 1 per loop cycle to give server breathing room
                    else:
                        print(f"[AI Generator] Error generating {template_idx}: {scenario['error']}")
                        await asyncio.sleep(10) # Wait before retry
                        break
            
            if not generated_any:
                await asyncio.sleep(30) # All pools are full
            else:
...
async def home(request: Request):
    ...
    try:
        r = await get_redis_client()
        total_msgs = await r.get("stats:total_msgs_global") or "0"
        total_users = await r.get("presence:total_global") or "0"
        public_stats = {
            "messages": f"{int(total_msgs):,}".replace(",", " "),
            "users": f"{int(total_users):,}".replace(",", " "),
            "days": 45
        }
    except:
...
async def get_sidebar_context(request: Request) -> Dict[str, Any]:
    ...
    ctx = {
        ...
        "training_url": f"http://{SERVER_IP}:{TRAINING_PORT}",
        "app_mode": APP_MODE
    }
    ...
    res = {
        ...
        "user": user,
        "is_admin": str(user.get("id")) in [str(x) for x in ADMIN_USER_IDS] if user else False
    }
    res.update(ctx)
    return res
...
@app.get("/login")
async def login_page(request: Request, target: str = "dashboard"):
    """Redirect to Discord OAuth with target state."""
    ...
    # Use target to determine where to go after auth
    state = target
...
async def auth_callback(request: Request, code: str = None, state: str = "dashboard", error: str = None):
    """Handle Discord OAuth callback with target redirect."""
    if error:
    ...
    # Store state for redirection after success
    if APP_MODE == "training":
        default_next = "/training"
    else:
    ...
    try:
        ...
        admin_str_list = [str(x) for x in ADMIN_USER_IDS]
        dynamic_str_list = [a.decode() if isinstance(a, bytes) else str(a) for a in dynamic_admins]
        
        user_id_str = str(user_data["id"])
        is_admin = user_id_str in admin_str_list or user_id_str in dynamic_str_list
        ...
        <body>
            <div class="container">
                ...
                <p>Přesměrování...</p>
                <script>setTimeout(function(){{ window.location.href = "{next_url}"; }}, 1000);</script>
            </div>
        </body>
        </html>
        """
...
async def training_page(request: Request, _=Depends(require_auth)):
    ...
    context = await get_sidebar_context(request)
...
async def get_training_scenarios(request: Request):
    """Load custom training scenarios from Redis."""
    try:
        r = await get_redis_client()
        scenarios_raw = await r.hgetall("training:custom_scenarios")
        scenarios = [json.loads(s) for s in scenarios_raw.values()] if scenarios_raw else []
...
async def get_training_history(request: Request):
    """Load training history for the logged-in user."""
    user = request.session.get("discord_user")
    if not user:
        return JSONResponse(content={"error": "K přístupu k historii se musíte přihlásit."}, status_code=401)
    
    user_id = user["id"]
    r = await get_redis_client()
    history_raw = await r.lrange(f"training:results:{user_id}", 0, -1)
    history = [json.loads(h) for h in history_raw]
    return JSONResponse(content={"history": history})

@app.get("/api/training/admin/stats")
async def get_admin_training_stats(request: Request):
    """Admin only: Load all training results for all users."""
    user_role = request.session.get("role")
    if user_role != "admin":
        return JSONResponse(content={"error": "Unauthorized"}, status_code=403)
    
    r = await get_redis_client()
    user_ids = await r.smembers("training:users")
    
    all_stats = []
    for uid in user_ids:
        history_raw = await r.lrange(f"training:results:{uid}", 0, -1)
        if history_raw:
            # We only need some metadata for the list, not full history
            history = [json.loads(h) for h in history_raw]
            # Get latest username from the last entry
            latest_user_info = history[-1].get("user", {})
            
            all_stats.append({
                "user_id": uid,
                "username": latest_user_info.get("username", "Unknown"),
                "avatar": latest_user_info.get("avatar"),
                "total_scenarios": len(history),
                "avg_score": round(sum(h.get("score", 0) for h in history) / len(history), 1),
                "last_active": history[-1].get("timestamp"),
                "history": history # Include history for detailed view
            })
    
    return JSONResponse(content={"stats": all_stats})

@app.post("/api/training/admin/generate")
async def admin_generate_scenario(request: Request):
    user_role = request.session.get("role")
    if user_role != "admin":
        return JSONResponse(content={"error": "Unauthorized"}, status_code=403)
    ...
    try:
        ...
        scenario = await generate_local_scenario(source)
        if "error" in scenario:
            return JSONResponse(content={"error": scenario["error"]}, status_code=500)
            
        return JSONResponse(content={"scenario": scenario})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)

@app.post("/api/training/generate")
async def user_generate_scenario(request: Request, _=Depends(require_auth)):
    """Allow any trainee to generate a scenario on-demand."""
    try:
        import random
        # Optional: frontend could pass the desired category, but default is random
        template_idx = random.randint(0, 5)
        
        r = await get_redis_client()
        popped = await r.lpop(f"training:scenario_pool:{template_idx}")
        
        if popped:
            scenario = json.loads(popped)
            print(f"[Training] Served pre-generated variant of template {template_idx}")
        else:
            print(f"[Training] Pool {template_idx} empty, generating on the fly...")
            scenario = await generate_local_scenario(template_idx)
            if "error" in scenario:
                return JSONResponse(content={"error": scenario["error"]}, status_code=500)
            
        # Immediately save it so it's ready in the system
        await r.hset("training:custom_scenarios", scenario["id"], json.dumps(scenario))
...
@app.post("/api/training/admin/save-scenario")
async def admin_save_scenario(request: Request):
    ...
    try:
        ...
        return JSONResponse(content={"success": True, "id": scenario["id"]})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)
...
async def get_training_courses(request: Request):
    ...
    try:
        ...
        if user:
            ...
            dynamic_admins = await r.smembers("training:admins")
            dynamic_str_list = [a.decode() if isinstance(a, bytes) else str(a) for a in dynamic_admins]
            is_admin = user_id in [str(x) for x in ADMIN_USER_IDS] or user_id in dynamic_str_list
        ...
        if not user:
            # Guests see public courses (id: 'public' or marked)
            visible = [c for c in courses if "guest" in c.get("allowed_roles", [])]
            return JSONResponse(content={"courses": visible})
            
        user_id = str(user["id"])
        user_rights_raw = await r.hget(f"training:user_rights", user_id)
        user_rights = json.loads(user_rights_raw) if user_rights_raw else {"roles": ["default"]}
        
        visible = []
        for c in courses:
            allowed = c.get("allowed_roles", [])
            # If user has ANY of the roles allowed for the course
            if any(role in allowed for role in user_rights.get("roles", [])):
                visible.append(c)
...
@app.post("/api/training/admin/courses")
async def admin_save_course(request: Request):
    ...
    try:
        ...
        r = await get_redis_client()
        await r.hset("training:courses", course["id"], json.dumps(course))
        return JSONResponse(content={"success": True, "id": course["id"]})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)

@app.delete("/api/training/admin/courses/{course_id}")
...
@app.get("/api/training/admin/users")
async def admin_get_users(request: Request):
    """List all users seen by the training system with detailed stats."""
    user_role = request.session.get("role")
    if user_role != "admin":
        return JSONResponse(content={"error": "Unauthorized"}, status_code=403)
    ...
    try:
        ...
        for uid_bytes in uids:
            ...
            # Basics from latest record
            latest = history[-1] if history else {}
            username = latest.get("user", {}).get("username", f"User {uid}")
            last_activity = latest.get("timestamp", "Never")
            
            # Performance Metrics
            scores = [h.get("evaluation", {}).get("score", 0) for h in history if h.get("evaluation")]
            avg_score = round(sum(scores) / len(scores), 1) if scores else 0
            
            # Admin Status (Hybrid: hardcoded + dynamic)
            is_admin_flag = int(uid) in ADMIN_USER_IDS or uid in [a.decode() if isinstance(a, bytes) else str(a) for a in dynamic_admins]
            
            users_list.append({
                "id": uid,
                "username": username,
                "roles": rights.get("roles", []),
...
@app.post("/api/training/admin/users/promote")
async def admin_promote_user(request: Request):
    ...
    try:
        ...
        r = await get_redis_client()
        await r.sadd("training:admins", target_uid)
        return JSONResponse(content={"success": True})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)

@app.post("/api/training/admin/users/demote")
async def admin_demote_user(request: Request):
    user_role = request.session.get("role")
    if user_role != "admin":
        return JSONResponse(content={"error": "Unauthorized"}, status_code=403)
    try:
        body = await request.json()
        target_uid = str(body.get("user_id"))
        if int(target_uid) in ADMIN_USER_IDS:
             return JSONResponse(content={"error": "Cannot demote hardcoded admin"}, status_code=400)
        r = await get_redis_client()
        await r.srem("training:admins", target_uid)
        return JSONResponse(content={"success": True})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)

@app.post("/api/training/admin/users/rights")
async def admin_set_user_rights(request: Request):
    """Set roles/access for a specific user."""
    user_role = request.session.get("role")
    if user_role != "admin":
        return JSONResponse(content={"error": "Unauthorized"}, status_code=403)
    ...
    try:
        body = await request.json()
        target_uid = str(body.get("user_id"))
        roles = body.get("roles", ["default"])
        
        r = await get_redis_client()
        await r.hset("training:user_rights", target_uid, json.dumps({"roles": roles}))
        return JSONResponse(content={"success": True})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)
...
async def evaluate_training(request: Request, _=Depends(require_auth)):
    """Algorithmic evaluation + optional AI comment. Instant, reliable, Czech-native."""
    try:
        body = await request.json()
        ...
        # ===== ALGORITHMIC EVALUATION (instant, no AI) =====
        from web.backend.evaluation_engine import evaluate_reply
        eval_data = evaluate_reply(user_reply, scenario_data)

        # ===== OPTIONAL: Try AI for a short bonus comment =====
        try:
            short_prompt = f"Jednou větou okomentuj tuto moderátorskou odpověď (skóre {eval_data['score']}/10): \"{user_reply[:200]}\""
            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.post(
                    "http://172.22.0.1:11434/api/generate",
                    json={
                        "model": "nepornu-expert",
                        "prompt": short_prompt,
                        "stream": False,
                        "options": {"temperature": 0.3, "num_predict": 80, "num_ctx": 512}
                    }
                )
                if resp.status_code == 200:
                    ai_comment = resp.json().get("response", "").strip()
                    if ai_comment and len(ai_comment) > 10:
                        eval_data["ai_comment"] = ai_comment[:200]
        except Exception:
            pass  # AI comment is optional — algorithmic score is the core
        ...
        if user:
            ...
            history_entry = {
                ...
                "user": {"id": user_id, "username": user["username"], "avatar": user.get("avatar")}
            }
            await r.rpush(f"training:results:{user_id}", json.dumps(history_entry))
            await r.sadd("training:users", user_id)
